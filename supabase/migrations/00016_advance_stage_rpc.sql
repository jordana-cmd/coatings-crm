-- 00016_advance_stage_rpc.sql
-- Server-side gate-checked stage advancement.
-- SECURITY DEFINER so it can write the locked stage column.
-- Ownership check inside prevents abuse.
...
BEGIN
  ...
  -- Ownership check: caller must own the opp or be admin
  ...
  -- ============================================================
  -- Validate transition is legal (no skipping, no backwards)
  -- ============================================================
  ...
  -- Target must be next active stage OR a valid terminal from current stage
  IF NOT (
  ...
  -- ============================================================
  -- Gate predicates (mirrors src/lib/gates/public-bid.ts EXACTLY)
  -- ============================================================
  ...
  IF v_opp.pipeline = 'PUBLIC_BID' THEN
    ...
    IF v_opp.stage = 'SOURCED' AND p_target_stage = 'ESTIMATING' THEN
      ...
      END IF;
      IF NOT v_bids.go_no_go THEN
        v_unmet := array_append(v_unmet, 'Go/No-Go decision made');
      END IF;
  ...
  IF v_opp.pipeline = 'PUBLIC_BID' AND p_target_stage = 'AWARDED' THEN
    -- CRITICAL: AWARDED stage = WON status (CLAUDE.md locked decision)
    v_new_status := 'WON';
  ELSIF p_target_stage = 'LOST' THEN
    v_new_status := 'LOST';
  ELSE
    v_new_status := v_opp.status; -- keep current (OPEN)
  END IF;
...
-- ============================================================
-- LOCKDOWN: stage column can only be changed via advance_stage
-- ============================================================

-- Remove table-level UPDATE (we'll re-grant column-by-column minus stage)
REVOKE UPDATE ON opportunities FROM authenticated;

-- Re-grant UPDATE on every column EXCEPT stage
GRANT UPDATE (
  id, name, pipeline, amount, owner_id, company_id,
  job_site_address, job_site_lat, job_site_lng,
  project_tag, prevailing_wage, status,
  lost_reason, revisit_date, created_at, updated_at
